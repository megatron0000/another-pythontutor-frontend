...
import { isStateType } from "./fraser-utils";
import { patchFraserInterpreterIfNotPatched } from "./fraser-patch";
...
export class Stepper {
  ...
  private interpreter: FraserInterpreter;
  ...
  constructor(
    ...
    patchFraserInterpreterIfNotPatched();
    ...
    if (typeof codeOrSerializedInternalState === "string") {
      ...
      this.interpreter = new FraserInterpreter(
        code,
    ...
    this.interpreter = new FraserInterpreter("", (interpreter, globalObject) =>
      this.injectFunctions(functions || [], interpreter, globalObject)
    );
  ...
  private injectFunctions(
    functions: [string, StepperInjectedFunction][],
    interpreter: FraserInterpreter,
    globalObject: FraserInterpreter.Object
  ) {
