...
export function createVisualizationController(
  ...
  const { heapId2View, frameId2FrameView, frameId2CodeAreaView, stepHeapRows } =
  ...
  for (const view of heapId2View.values()) {
    view.onWillDrag(delta => {
      ...
      const scale = parseFloat(
        visualizerContainer.style.transform.match(/scale\(([^)]+)\)/)?.[1] ||
          "1"
      );
      delta.dx *= 1 / scale;
      delta.dy *= 1 / scale;
  ...
  function renderStep(stepIndex: number, prevStepIndex: number) {
    ...
    const currentStackedFrameIds = step.stack_frames
      .slice(0, -1)
      .map(x => x.frame_id);
    for (let pastStepIndex = 0; pastStepIndex < stepIndex; pastStepIndex++) {
    ...
    currentViews.forEach(heapElementView => {
      ...
      const position =
        draggedPosition !== undefined ? draggedPosition : calculatedPosition!;
