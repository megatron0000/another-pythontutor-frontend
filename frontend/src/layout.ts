...
define(["d3/d3.v7", "./diff"], function (
  ...
  function calculateConnections(
    ...
    const pointer2View = buildPointer2View(
      step,
      heapId2View,
      frameId2FrameView
    );
  ...
  function createContextLayouter(container: HTMLElement | null) {
    if (container === null) {
  ...
  class ConnectionRouterImplementation implements ConnectionRouter {
    ...
    rerender(connections: Connection[]): void {
      ...
      const { destroyed, created, updated } = diff.diffConnections(
  ...
  function createConnectionRouter(jsplumb: BrowserJsPlumbInstance) {
