...
export function createShallowObjectView(
  objectData: HeapObject
): HeapElementView {
...
export function createHeapFunctionView(code: string): HeapElementView {
  const heapElementContainer = d3
    .create("div")
    .classed("heap__element__container", true);
...
export function createShallowStackFrameView(frame: StackFrame): StackFrameView {
  const key2anchorOut: Map<string, HTMLElement> = new Map();
  ...
  view.rerender = function rerender(newData: StackFrame) {
    ...
    newFrameTrCollection
      .append("td")
      .classed("stack__frame__body__variable__identifier", true)
      .classed("return_value_identifier", datum => datum === RETURN_VALUE_NAME)
      .text(datum => datum);
...
function draggableBehaviour_factory(view: HeapElementView) {
  ...
  const onWillDragHandlers: Array<(delta: { dx: number; dy: number }) => void> =
    [];
